...
# Create a fallback poster system
def get_fallback_poster(movie_title, genre=None):
    """Generate a simple text-based poster as fallback"""
    try:
        # Create a simple colored background based on genre
        color_map = {
            'action': '#FF6B6B',
            'comedy': '#4ECDC4', 
            'drama': '#45B7D1',
        ...
        # Try to detect genre from movie data if available
        if genre is None:
            try:
        ...
        # Create HTML for a simple poster
        poster_html = f"""
        <div style="
            width: 100%;
            height: 400px;
...
def fetch_poster(movie_id, movie_title=None, use_fallback=True):
    """
    Fetch poster with fallback options
    """
    # Try TMDB API first
    if not use_fallback:
...
def recommend(movie=None, era=None, genre=None):
    ...
    # Sort by similarity score if movie-based recommendation
    if movie:
                ...
                for j in range(3):
                    if i + j < len(titles):
                        with cols[j]:
                            ...
                            # Get movie details
                            movie_details = movies_data.iloc[i + j]
                            year = movie_details.get('release_year', 'N/A')
                            ...
                            # Movie info
                            wiki_link = build_wikipedia_link(movie_title)
                            st.markdown(f"**[{movie_title}]({wiki_link})**")
                            st.caption(f"📅 {year} | 🎭 {genres[:30]}...")
...
if demo_mode:
    st.sidebar.success("✅ Ready for interview demo!")
else:
